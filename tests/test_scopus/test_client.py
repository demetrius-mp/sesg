...
@pytest.mark.asyncio
async def test_scopus_search_should_retry_fetch_when_key_error_error_occurs(
    httpx_mock: HTTPXMock,
):
    httpx_mock.add_response(
        200,
        url="https://api.elsevier.com/content/search/scopus/?apiKey=k1&query=test&start=0",
        json={"not-search-results": {}},
    )
...
@pytest.mark.asyncio
async def test_scopus_search_should_retry_at_most_5_times_when_key_error_occurs(
    httpx_mock: HTTPXMock,
):
    for i in range(1, 6):
        httpx_mock.add_response(
...
@pytest.mark.asyncio
async def test_scopus_search_should_retry_at_most_3_times_when_key_error_occurs(
    httpx_mock: HTTPXMock,
):
    client_module.MAX_ATTEMPTS_ON_KEY_ERROR = 3
...
@pytest.mark.asyncio
async def test_scopus_search_should_retry_fetch_when_internal_error_error_occurs(
    httpx_mock: HTTPXMock,
):
    httpx_mock.add_response(
        500,
...
@pytest.mark.asyncio
async def test_scopus_search_should_retry_at_most_5_times_when_internal_error_occurs(
    httpx_mock: HTTPXMock,
):
    for i in range(1, 6):
        httpx_mock.add_response(
...
@pytest.mark.asyncio
async def test_scopus_search_should_retry_at_most_3_times_when_internal_error_occurs(
    httpx_mock: HTTPXMock,
):
    client_module.MAX_ATTEMPTS_ON_SCOPUS_INTERNAL_ERROR = 3
